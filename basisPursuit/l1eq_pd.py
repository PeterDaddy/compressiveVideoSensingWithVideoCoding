...
def l1eq_pd(x0, A, At, b, pdtol=1e-3, pdmaxiter=50, cgtol=1e-8, cgmaxiter=200, verbose=False):
    # This line is use to immitate how MATLAB work with true divide.
    # Numpy and SciPy hate this but there is not so much choice for optimization
    ################# Warning:This will set true divide [off] ###################
    ...
    while not done:
      ...
      if largescale:
        ...
        #[dv,hcond] = linsolve(H11p, w1p, opts);
        try:
          dv = numpy.linalg.solve(H11p, w1p)
